...
export function BookingForm({ campsite }: BookingFormProps) {
  ...
  const { nights, totalPrice } = useMemo(() => {
    ...
    if (campsite.price_unit === 'entrance') {
      return { nights: 0, totalPrice: unit * guests };
    }
    return { nights: n, totalPrice: unit * n * guests };
        ...
        <div className="flex justify-between items-center text-sm text-green-800">
          <span>
            {campsite.price_unit === 'entrance'
              ...
              : `₱${campsite.price_per_night} × ${nights} ${nights === 1 ? 'night' : 'nights'} × ${guests} ${guests === 1 ? 'guest' : 'guests'}`}
          </span>
          <span className="font-semibold">
            ₱{(parseFloat(campsite.price_per_night) * (nights || 1) * guests).toFixed(2)}
          </span>
        </div>
